...
class Command_grep(HoneyPotCommand):
    ...
    def grep_get_contents(self, filename: str, match: str) -> None:
        try:
            ...
            self.grep_application(contents, match, filename)
        except Exception:
            self.errorWrite(f"grep: {filename}: No such file or directory\n")

    def grep_application(self, contents: bytes, match: str, filename: str) -> None:
        ...
        for line in contentsplit:
            if matches.search(line):
                if filename:
                    self.writeBytes(f'{filename}:'.encode('utf-8'))
    ...
    def expand_glob(self, path: str) -> list[str]:
        if "*" in path or "?" in path or "[" in path:
            dirname = os.path.dirname(path) or "."
            pattern = os.path.basename(path)
            try:
                files = self.fs.listdir(dirname)
                return [
    ...
    def start(self) -> None:
        ...
        try:
            optlist, args = getopt.getopt(
                self.args, "abcDEFGHhIiJLlmnOoPqRSsUVvwxZA:B:C:e:f:r"
            )
        ...
        if not self.input_data:
            if self.recursive:
                for path in paths:
                    self.grep_recursive(path, pattern)
            else:
                expanded_files = []
                for path in paths:
                    expanded_files.extend(self.expand_glob(path))
                for file in expanded_files:
                    self.grep_get_contents(file, pattern)
        else:
            self.grep_application(self.input_data, pattern, args[1])

        self.exit()

    def grep_recursive(self, path: str, pattern: str) -> None:
        if self.fs.isfile(path):
            self.grep_get_contents(path, pattern)
        elif self.fs.isdir(path):
            for entry in self.fs.listdir(path):
                if entry in (".", ".."):
                    continue
                full_path = os.path.join(path, entry)
                self.grep_recursive(full_path, pattern)
