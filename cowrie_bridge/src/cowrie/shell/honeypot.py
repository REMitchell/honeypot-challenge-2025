...
class HoneyPotShell:
    ...
    def _start_om_thread(self):
        ...
        log.msg(f"DEBUG: OPENAI_API_KEY = {os.environ.get('OPENAI_API_KEY')}")
        log.msg(f"DEBUG: ASSISTANT KEY = {os.environ.get('OPENAI_ASSISTANT_ID_BRIDGE')}")

        def background_task():
            try:
                session = self.protocol.user
                thread = getattr(session, "thread", None) or openai.beta.threads.create()
                session.thread = thread

                openai.beta.threads.messages.create(
                    thread_id=thread.id,
                    role="user",
                    content=self.last_raw_line,
                )
                run = openai.beta.threads.runs.create(
                    thread_id=thread.id,
                    assistant_id=os.environ["OPENAI_ASSISTANT_ID_BRIDGE"],
                )
        ...
        def on_complete(result):
            ...
            for msg in [m for m in messages.data if m.role == 'assistant']:
                for blk in [b for b in msg.content if b.type == 'text']:
                    self.write_om_response(blk.text.value)
                break
            else:
